...
{
	static GGWave::CBWaveformOut cb_waveform_output = [&](const void* data, uint32_t bytes) {
		m_device_output.queue_audio(data, bytes);
	};

	static GGWave::CBWaveformInp cb_waveform_input = [&](void* data, uint32_t max_bytes) {
	...
	if (m_ggwave_ptr->hasTxData()) {
		...
		m_ggwave_ptr->encode(cb_waveform_output);
	} else {
		...
		if (m_device_output.queued_audio_size() >= static_cast<uint32_t>(m_ggwave_ptr->getSamplesPerFrame() * m_ggwave_ptr->getSampleSizeBytesOut())) {
			tLastNoData = tNow;
		}
		else {
			...
			if (getTime_ms(tLastNoData, tNow) > 500.0f) {
				m_ggwave_ptr->decode(cb_waveform_input);
				if (m_device_input.queued_audio_size() > static_cast<uint32_t>(32 * m_ggwave_ptr->getSamplesPerFrame() * m_ggwave_ptr->getSampleSizeBytesInp())) {
...
AudioDevice Ultrasonic::init_device_output()
{
	...
	playback_spec.callback = nullptr;
